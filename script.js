...
const sanitizeCaseEntry = (entry) => {
  ...
  const disablePlayer = entry.disablePlayer === true;

  if (!src && !disablePlayer) {
    return null;
  }
...
const buildPortfolioItemElement = (data) => {
  const isPlayable = !!data && data.disablePlayer !== true;

  if (!data || (!data.src && isPlayable)) {
    return null;
  }
  ...
  if (isPlayable && data.src) {
    article.setAttribute('data-video-src', data.src);
  }

  if (!isPlayable) {
    article.setAttribute('data-disable-player', 'true');
  }

  const videoId = data.src ? extractYouTubeId(data.src) : '';
...
const TITLE_THUMBNAIL_OVERRIDES = {
  ...
  'eucalypso home': 'https://i.ibb.co/5x5wG4Wr/IMG-1285.jpg',
};

const DESKTOP_TABLET_TITLE_THUMBNAIL_OVERRIDES = {
  'america business forum 2025': 'https://i.ibb.co/b5d30tGq/1-B01160-B-4181-44-BF-B5-F9-2248191-DE4-AD.jpg',
  'american business forum 2025': 'https://i.ibb.co/b5d30tGq/1-B01160-B-4181-44-BF-B5-F9-2248191-DE4-AD.jpg',
  'america business forum 2025 / highlights': 'https://i.ibb.co/b5d30tGq/1-B01160-B-4181-44-BF-B5-F9-2248191-DE4-AD.jpg',
  'american business forum 2025 / highlights': 'https://i.ibb.co/b5d30tGq/1-B01160-B-4181-44-BF-B5-F9-2248191-DE4-AD.jpg',
  ozora: 'https://i.ibb.co/CpqwdVrf/IMG-1289.jpg',
  'gross hunter': 'https://i.ibb.co/DDhyX7hb/IMG-1283.jpg',
  'star wars shadow of justice': 'https://i.ibb.co/xK5b5pft/IMG-1282.jpg',
  'eucalypso home commercial': 'https://i.ibb.co/5x5wG4Wr/IMG-1285.jpg',
  'eucalypso home': 'https://i.ibb.co/5x5wG4Wr/IMG-1285.jpg',
...
const applyThumbnailOverrides = (item) => {
  ...
  const isMobile = viewportWidth && viewportWidth <= 640;

  const override =
    (videoId && VIDEO_THUMBNAIL_OVERRIDES[videoId]) || resolveTitleOverride(title, TITLE_THUMBNAIL_OVERRIDES);

  const desktopOverride = !isMobile
    ? resolveTitleOverride(title, DESKTOP_TABLET_TITLE_THUMBNAIL_OVERRIDES)
    : '';

  const chosenOverride = desktopOverride || override;

  if (chosenOverride) {
    setCustomThumbnail(item, chosenOverride);
  }
};
...
portfolioItems.forEach((item) => {
  const isDisabled = item.getAttribute('data-disable-player') === 'true';
